...
function tabActiveReset(tab) {
  return Array.from(tab.parentNode.parentNode.querySelectorAll('li'))
    .forEach(element => element.classList.remove('slds-active'))
}
const tabActiveSet = tab => tab.parentNode.classList.add('slds-active')
function tabPanelsReset(tab) {
  return Array.from(tab.parentNode.parentNode.parentNode.querySelectorAll('[role="tabpanel"]'))
    .forEach((tabpanel) => {
...
function assignTabEvents(event) {
